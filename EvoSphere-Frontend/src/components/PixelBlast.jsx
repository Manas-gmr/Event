...
const createTouchTexture = () => {
  ...
  const update = () => {
    ...
    for (let i = trail.length - 1; i >= 0; i--) {
      const point = trail[i];
      const f = point.force * (1 / maxAge) * (1 - point.age / maxAge);
      point.x += point.vx * f;
      point.y += point.vy * f;
      point.age++;
...
float Bayer2(vec2 a) {
  a = floor(a);
  return fract(a.x / 2. + a.y * a.y * 0.75);
}
...
void main() {
  ...
  vec2 cellId = floor(fragCoord / (8.0 * pixelSize));
  ...
  if (uEnableRipples == 1) {
    for (int i = 0; i < MAX_CLICKS; ++i) {
      ...
      vec2 cuv = (((pos - uResolution * 0.5 - vec2(8.0 * pixelSize * 0.5)) / uResolution)) * vec2(aspectRatio, 1.0);
      float t = max(uTime - uClickTimes[i], 0.0);
      float r = distance(uv, cuv);
      float waveR = uRippleSpeed * t;
      float ring = exp(-pow((r - waveR) / uRippleThickness, 2.0));
      float atten = exp(-10.0 * t) * exp(-10.0 * r);
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          'NoiseEffect',
          `uniform float uTime; uniform float uAmount; void mainUv(inout vec2 uv) {} void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) { float n = fract(sin(dot(floor(uv * vec2(1920.0,1080.0)) + floor(uTime * 60.0), vec2(127.1, 311.7))) * 43758.5453); float g = (n - 0.5) * uAmount; outputColor = inputColor + vec4(vec3(g), 0.0); }`,
          {
            uniforms: new Map([
              ['uTime', new THREE.Uniform(0)],
